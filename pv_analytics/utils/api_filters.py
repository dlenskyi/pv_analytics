...
class MeterP30DataFilter(filters.FilterSet):
    date_start = filters.CharFilter(field_name="date", lookup_expr="gte")
    ...
    def site_filter(self, queryset, name, value):
        ...
        return queryset.using("remote").filter(meter__site__displayable_name__in=sites)

    def device_filter(self, queryset, name, value):
        return queryset.using("remote").filter(Q(meter__name__icontains=value) | Q(meter__id__icontains=value))
...
class BalancesFilter(filters.FilterSet):
    ...
    def site_filter(self, queryset, name, value):
        ...
        # all sites that are in this list
        sites = self.request.query_params.getlist("site")
        return queryset.using("remote").filter(site__displayable_name__in=sites)
