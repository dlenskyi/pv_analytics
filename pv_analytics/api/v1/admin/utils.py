...
def update_ordered_dict(old_dict):
    ...
    """
    return OrderedDict([(re.sub("\\D", "", k), v) if k.startswith("number_") else (k, v) for k, v in old_dict.items()])
...
def calculate_balance(meter_p30_data_id, corrected_meter_p30_data_values):
    ...
    """
    try:
        first_meter_p30_data = MeterP30Data.objects.using("remote").get(id=meter_p30_data_id)
    ...
    if first_meter_inst.parameter in parameters_for_balance:
        ...
        second_meter_p30_data = (
            MeterP30Data.objects.using("remote")
            .filter(
            ...
            .exclude(meter__parameter=first_meter_inst.parameter)
            .first()
        )
        ...
        if first_meter_inst.parameter == generation_parameter:
            total_paired_values = zip(first_paired_values_sum, second_paired_values_sum)
        else:
            total_paired_values = zip(second_paired_values_sum, first_paired_values_sum)
        ...
        balance = (
            Balance.objects.using("remote")
            .filter(site=first_meter_inst.site, date=first_meter_p30_data.date)
